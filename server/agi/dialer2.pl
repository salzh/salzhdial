...
if ($status eq 'INCREASECALLS') {
	$agi->verbose("update t_ccalls set ccalls=ccalls+1",1);
	$dbh->prepare("update t_ccalls set ccalls=ccalls+1")
		->execute();
	exit 0;
}
...
if ($status eq 'DIALERHANGUP') {
	...
	$agi->verbose("update t_ccalls set ccalls=ccalls-1 where ccalls > 0", 1);
	$dbh->prepare("update t_ccalls set ccalls=ccalls-1 where ccalls > 0")
		->execute();;
...
=cut
	...
	$sql = "update t_work set overcount=overcount+1 where id='$workid' and overcount < workcount";
	log_debug(3, "SQL", $sql);
...
sub asterisk_collect_digit {
        ...
        if ($prompt ne "") {
			...
			if (!$nowait && $flags && $digit_code eq 0) {
                $digit_code = $agi->wait_for_digit('10000');
            }
