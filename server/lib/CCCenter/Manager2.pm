...
CCCenter::Manager - CCCenter Manager Interface
...
This module provides a simple interface to the cccenter manager interface.
...
sub connect {
	...
	if ($self->iso->can_read(0.5)) {	
		...
		return $self->error("CCCenter return Nothing");
	}
	...
	if ($manager !~ /CCCenter Call Manager/) {
...
sub sendcommand {
	...
	if ($self->iso->can_write()) {
		...
		if (!$bytes) {
			carp "write nothing to cccenter socket!!!\n";
			return;
		}
		#print $cstring.$EOL if $self->debug;
	} else {
		carp "cannot write to cccenter socket!!!\n";
		return;
	}
