package CCCenter::Outgoing;

=head1 NAME

CCCenter::Outgoing - Create outgoing call queue file

=head1 SYNOPSIS

use CCCenter::Outgoing;

my $out = new CCCenter::Outgoing;
...
sub new {
	my ($class, %args) = @_;
	my $self = {};
	$self->{OUTDIR} = '/var/spool/cccenter/outgoing';
	$self->{OUTTIME} = undef;
	$self->{OUTVARS} = {};
	$self->{CUSTOM_VARS} = ();
