package CCCenter::Voicemail;

require 5.004;

=head1 NAME

CCCenter::Voicemail - Stuff to deal with cccenter voicemail
...
sub spooldirectory {
	...
	if (defined($directory)) {
		...
		$self->{'spooldirectory'} = '/var/spool/cccenter/vm' if (!defined($self->{'spooldirectory'}));
	}
...
sub sounddirectory {
	...
	if (defined($directory)) {
		...
		$self->{'sounddirectory'} = '/var/lib/cccenter/sounds' if (!defined($self->{'sounddirectory'}));
	}
...
sub configfile {
	...
	if (defined($configfile)) {
		...
		$self->{'configfile'} = '/etc/cccenter/voicemail.conf' if (!defined($self->{'configfile'}));
	}
