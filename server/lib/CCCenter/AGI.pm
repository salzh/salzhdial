...
$VERSION = $CCCenter::VERSION;

=head1 NAME

CCCenter::AGI - Simple CCCenter Gateway Interface Class

=head1 SYNOPSIS

use CCCenter::AGI;

$AGI = new CCCenter::AGI;
...
cccenter open source pbx via AGI (cccenter gateway interface)
...
Deletes a family or specific keytree within a family in the CCCenter database
...
Does absolutely nothing except pop up a log message.  
 Useful for outputting debugging information to the CCCenter console.
...
This command instructs CCCenter to play the given sound file and listen for the given dtmf digits. The
fileextension must not be used in the filename because CCCenter will find the most appropriate file
