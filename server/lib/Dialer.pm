		...
		if ($j >= $total || time() - $start >= AUTODIALINT) {
			...
			if (!$total) {
				...
				$dbh->prepare("update t_livetable set livecalls=?,maxcalls=? where host=?")
					->execute(0, 0, 'sending');
				$dbh->prepare("insert t_livelog (host,num) values (?, ?)")
					->execute('sending', 0);
				$dbh->prepare("update t_ccalls set ccalls=0")->execute();

#update t_work set workstate=3,overcount=workcount where workstate=1
		...
		if (@spool) {
			for my $row (@spool) {
				...
				for (1..60) {				
					...
					$dbh->prepare("insert t_livelog (host,num) values (?, ?)")
						->execute($main::dispatch_conf{BRANCHNAME}, $ccalls);	
			...
			#	$dbh->prepare("update t_ccalls set ccalls=ccalls+1")->execute();
				if ($username eq 'liuchuangyei') {
...
sub getcurrentcalls2 {
	my $dbh = shift || return 999;
	my $sth = $dbh->prepare("select ccalls from t_ccalls");
	$sth   -> execute();
	
	my $row = $sth->fetchrow_hashref;
	return (defined $row->{ccalls} ? $row->{ccalls} : 999);
}
