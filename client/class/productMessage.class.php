<?php 
	...
	class Message{
		...
		{
			$sql="select id from t_product_message a  where 1=1 ";
			if($sqlwhere!='')
			{
				$sql=$sql. $sqlwhere;
			}
		...
		public function AddReplay($MessageInfo)
		{
			...
			try {
				$this->conn->update("t_product_message","state=1","id=".$MessageInfo->Id);
		...
		public function GetPageList($sqlwhere='',$page=1,$pagesize=20)
		{
			...
			$rss=array();
			$sql="select  a.*,b.title  from t_product_message  a left join t_news b on b.id=a.newsid ";
 	...
 	///返回分类列表
		...
		public function GetModel($newsid)
		{
			...
			try {  
				...
				$sql="select * from t_product_message where id=".$newsid ;	
				$result=$this->conn->query($sql);
