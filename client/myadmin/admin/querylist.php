...
<link href="../css/public.css" rel="stylesheet" type="text/css" />
	...
	<SCRIPT type="text/javascript">
		...
		function onTreeClick(event, treeId, treeNode, clickFlag) {
			$("#listdetail").attr("src","rightquerylist.php?upid="+treeNode.id+"&uname=<?=$uname?>&sdate=<?=$sdate?>&edate=<?=$edate?>");
...
if ($a=="del"){
	...
	if($ad->Del($id)){
	echo Msg("操作成功","querylist.php?action=list");
	}
}
?>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="12%" height="45" style="border-bottom:2px #218644 solid">&nbsp;<span class="title"><?=$pagetitle?></span></td>
    <td width="88%" height="45" style="border-bottom:2px #218644 solid"><form id="form1" name="form1" method="post" action="?">
      <div align="right">开始时间：<input type="text" name="sdate" class="ui_timepicker" value="<?=$sdate?>">&nbsp;截止时间：<input type="text" name="edate" class="ui_timepicker" value="<?=$edate?>">&nbsp;员工名称：<input name="uname" type="text" class="inputSearch" id="uname" value="<?=$uname?>"/>排序规则：<?=$fun->GetComboList("idx",5,$idx)?>
...
<form id="form2" name="form2" method="post" action="?action=delall" onsubmit="return confirm('确定要删除吗，删除后不可恢复');">
  ...
  <td width="80%" valign="top">
	<iframe id="listdetail" src="rightquerylist.php?uname=<?=$uname?>&sdate=<?=$sdate?>&edate=<?=$edate?>" frameborder="0" width="100%" height="550"></iframe>
  </td>
  </tr>
  </table>
