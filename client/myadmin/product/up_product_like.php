 	...
 	$picid= isset($_REQUEST['picid']) ? $_REQUEST['picid'] : '0';
 		...
 		if($a=="addsave" || $a=="modifysave")
		{
			...
			{	
				...
				if($ppic->AddProductLike($ppicinfo)>0)
				{
				 	$pro->AddLog($_COOKIE['username'],$productid,$pro->GetTitle($productid),"添加 [".$pro->GetTitle($productid)."] 相关产品信息");	

					echo Msg("操作成功","up_product_like.php?action=add&id=$productid");
				}
			...
			if ($a=="modifysave")
			{
				...
				if($picid>0)
				{
					...
					if($ppic->EditProductLike($ppicinfo)>0)
					{
						$pro->AddLog($_COOKIE['username'],$productid,$pro->GetTitle($productid),"修改 [".$pro->GetTitle($productid)."] 相关产品信息");	
						 echo Msg("操作成功","up_product_like.php?action=add&id=$productid");
					}
					else
					{
			...
			if ($a=="del"){
				$picid= isset($_REQUEST['picid']) ? $_REQUEST['picid'] : '0';
				if($ppic->DelProductLike($picid)){
				 $pro->AddLog($_COOKIE['username'],$id,$pro->GetTitle($id),"删除[".$pro->GetTitle($id)."] 相关产品信息");
				echo Msg("操作成功","up_product_like.php?action=add&id=$id");
				}
			}
...
<div class="box">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" >
    <tr>
      <td width="75%" height="45" style="border-bottom:2px #0066CC solid">&nbsp;<span class="title">产品管理&gt;相关产品</span></td>
    ...
    <table width="500" height="37" border="0" cellpadding="0" cellspacing="2" bgcolor="#FFFFFF">
      <tr>
        ...
        <td width="100" bgcolor="#0099FF"><div align="center" style="color:#999999; cursor:pointer;" ><a href="up_product_like.php?id=<?=$_REQUEST['id']?>" style="color:#FFFFFF">相关产品</a></div></td>
        ...
        <td height="10">	<a href="?action=add&id=<?=$id?>" style="font-size:15px; margin:5px; background:#efefef; text-align:center; display:block; width:100px; height:30px; line-height:30px;">添加</a>
          <table width="600" height="31" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td width="83" height="31" bgcolor="#efefef"><div align="center"> 操 作</div></td>
            </tr>
          </table>
          <br />
		
		<?
			$listpic = $ppic->GetlistLike($id,0,100,"");
			if($listpic!=NULL)
			{
		...
		?>
		 ...
		 <table width="600" height="29" border="0"  cellpadding="0" cellspacing="0" style="border-bottom:1px solid #ccc;">
           <tr>
             ...
             <td width="446"><a href="up_product_like.php?id=<?=$id?>&amp;picid=<?=$rspic['id']?>&amp;action=modify">
               <?=$rspic["title"]?>
             </a></td>
             <td width="83" height="29"><div align="left"><a href="up_product_like.php?id=<?=$id?>&amp;picid=<?=$rspic['id']?>&amp;action=modify">修改</a>&nbsp;&nbsp;&nbsp; <a href="?action=del&picid=<?=$rspic['id']?>&id=<?=$id?>" onclick="return confirm('确定要删除吗，删除后不可恢复');"> 删除</a></div></td>
           </tr>
         </table>
...
?>  
    ...
    <table width="100%" height="190" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
      ...
      <tr>
        <td height="36" colspan="2" bgcolor="#efefef">添加相关产品</td>
      </tr>
      <tr>
...
<?
	...
	{
		$picid=isset($_REQUEST['picid']) ? $_REQUEST['picid'] : '0';
	 	$rs=$ppic->GetClassModelLike($picid);
 		if ($rs->Id!=0)
		{
?>  
