...
<link href="../css/public.css" rel="stylesheet" type="text/css" />
	...
	<SCRIPT type="text/javascript">
		...
		function onTreeClick(event, treeId, treeNode, clickFlag) {
			$("#listdetail").attr("src","rightsublist.php?upid="+treeNode.id+"&uname=<?=$uname?>&SearchType=<?=$SearchType?>&sdate=<?=$sdate?>&edate=<?=$edate?>");
			//$("#treedetail").html("[ onClick ]&nbsp;&nbsp;clickFlag = " + treeNode.id + " (" + (clickFlag===1 ? "普通选中": (clickFlag===0 ? "<b>取消选中</b>" : "<b>追加选中</b>")) + ")");
			/*
			$.ajax({
				type : "POST",
				url : 'rightsublist.php?upid='+treeNode.id,
				dataType : "html",
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="7%" height="45" style="border-bottom:2px #218644 solid">&nbsp;<span class="title">管理员管理</span></td>
    <td width="93%" height="45" style="border-bottom:2px #218644 solid"><form id="form1" name="form1" method="post" action="?">
      <div align="right">
        开始时间：
...
<form id="form2" name="form2" method="post" action="?action=delall" onsubmit="return confirm('确定要删除吗，删除后不可恢复');">
  ...
  <table width="100%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#999999" height="550">
	...
	<iframe id="listdetail" src="rightsublist.php?uname=<?=$uname?>&SearchType=<?=$SearchType?>&sdate=<?=$sdate?>&edate=<?=$edate?>" frameborder="0" width="100%" height="550"></iframe>
  </td>
  </tr>
  </table>
</form>
