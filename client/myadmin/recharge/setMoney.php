...
<script type="text/javascript">
	...
	function check(){
		...
		if(uname.Trim()==""){alert("请输入充值账号");$Id("username").focus();return false;}
		var uname=$Id("GroupName").value;
		if(uname.Trim()==""){alert("请输入充值金额");$Id("GroupName").focus();return false;}
	}
	var $j=jQuery.noConflict();
	var $Id=function(id){return document.getElementById(id);}
...
if($ac=="addsave"){
	...
	$thisModel->Description="[".$_SESSION['username']."给".$uname."充值".$RechargeMoney."元]".$Description;
	...
	else
	{
		...
		$allmoney=0;
		if($adminInfoParent->id>0)
		{
			$allmoney=$adminInfoParent->voiceMoney;
		}
...
<div class="box">
  ...
  <form id="form1" name="form1" method="post" action="?action=addsave" onsubmit="return check()">
    <table width="100%" height="128" border="0" cellpadding="1" cellspacing="1" bgcolor="#CCCCCC">
      <tr>
        <td height="28" align="center" bgcolor="#FFFFFF">充值账号</td>
        <td height="28" bgcolor="#FFFFFF"><input type="text" name="uname" id="uname" value="<?=$uname?>" maxlength="50" />
          必填项</td>
      </tr>
      <tr>
        <td height="28" align="center" bgcolor="#FFFFFF">充值金额</td>
